...
- =graphics= ::
  ...
  The code block produces graphics, which are saved to the file
  specified in the =:file= header argument, and the result is a link
  to that file.  Graphics implementation varies by language, so see
  the language-specific documentation for further details on how to
  use this header arg.
